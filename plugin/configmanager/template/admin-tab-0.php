...
<form id="configForm" method="post" action="index.php?p=configmanager&action=save">
  ...
  <div class="row">
    <div class="large-12 columns">
      <input <?php if($config['hideTitles']){ ?>checked<?php } ?> type="checkbox" name="hideTitles" /> <label for="hideTitles"><?php echo lang("Hide pages titles"); ?>
    </div>
  ...
  <div class="row">
    <div class="large-12 columns">
      ...
      <?php foreach($themes as $k=>$v){ ?>
	    <li><input type="radio" name="theme" <?php if($k == $config['theme']){ ?>checked<?php } ?> value="<?php echo $k; ?>" /> <label for="theme"><?php echo $v['name']; ?> <a class="label secondary round" target="_blank" href="<?php echo $v['authorWebsite']; ?>"><?php echo $v['authorWebsite']; ?></a></label></li>
	    <?php } ?>
      </ul>	    
    </div>
  </div>
